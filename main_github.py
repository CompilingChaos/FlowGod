...
async def process_ticker_sequential(ticker, macro_context):
...
async def scan_cycle():
    ...
    # 2. Global Macro Context (Fetch once per scan)
    ...
    for ticker in watchlist:
        alerts_data = await process_ticker_sequential(ticker, macro)
        
        for data in alerts_data:
            trade = data['trade']
            context = data['context']
            macro_context = data['macro']

            logging.info(f"Analyzing potential whale for {trade['ticker']}...")

            # send_alert now handles context-aware reasoning
            sent = await send_alert(trade, context, macro_context)
        ...
        # 4. Mandatory delay between tickers
